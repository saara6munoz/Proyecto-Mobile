...
export class DbService {
  ...
  constructor(private sqlite: SQLite) {
    this.sqlite.create({
      ...
      .then((db: SQLiteObject) => {
        db.executeSql('CREATE TABLE IF NOT EXISTS PERSONA(NOMBRE VARCHAR(20), APELLIDO VARCHAR(20), EDAD INTEGER)', [])
  ...
  almacenarPersona(nombre: string, apellido: string, edad: number) {
    this.sqlite.create({
      name: 'data.db',
      location: 'default'
    })
      .then((db: SQLiteObject) => {
        db.executeSql('INSERT INTO PERSONA VALUES(?, ?, ?)', [nombre, apellido, edad])
  ...
  obtenerTodasLasPersonas() {
    return this.sqlite.create({
      ...
      .then((db: SQLiteObject) => {
        return db.executeSql('SELECT NOMBRE, APELLIDO, EDAD FROM PERSONA', [])
          .then((data) => {
            let lista_personas = [];
